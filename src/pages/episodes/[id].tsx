    ...
    <Container>
      ...
      <Content>
          ...
          <Link href="/">
            <Tooltip title="Retornar para página inicial" arrow>
              <Button className="button">
                <img src="/svg/arrow-left.svg" alt="Botão para iniciar o episódio de Podcast" />
              </Button>
            </Tooltip>
          </Link>
          <Image src={episode.thumbnail} width="994" height="320" objectFit="cover" className="imgPodcast" />
          <Tooltip title={`Play no episódio ${episode.title}`} arrow>
            <Button className="button">
              <img src="/svg/play.svg" alt="Botão para iniciar o episódio de Podcast" onClick={() => play(episode)} />
            </Button>
          </Tooltip>
        </div>
        <div className="bodyPagePodcast">
          ...
          <div className="descriptionEpisodePodcast" dangerouslySetInnerHTML={{ __html: episode.description }} />
        </div>
      </Content>
    </Container>
  );
...
export const getStaticProps = async ({ params }: ContextParamsGetStaticProps) => {
  const { id } = params;
